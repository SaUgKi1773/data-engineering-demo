...
<div class="flex flex-col rounded-xl border border-gray-200 bg-white px-2 py-1.5">
  <div class="truncate text-[11px] font-semibold uppercase tracking-wider text-gray-500">{title}</div>

  {#if rows.length}
    <div class="mt-0.5 flex items-baseline gap-1">
      <span class="h-2 w-2 flex-none rounded-full" style="background:{rows[0].colour}"></span>
      <span class="text-[12px] font-semibold text-gray-500">{rows[0].code}</span>
      <span class="text-[17px] font-semibold leading-none tabular-nums text-gray-900">{format(rows[0].value)}</span>
    </div>
    ...
    <div class="mt-0.5 flex gap-1">
      {#each rows as r}
        <span class="flex-1 text-center text-[11px] font-medium leading-tight text-gray-500">{r.code}</span>
      {/each}
    </div>
    <div class="flex gap-1">
      {#each rows as r, i}
        <span class="flex-1 text-center text-[11px] leading-tight tabular-nums {i === 0 ? 'font-semibold text-gray-900' : 'text-gray-400'}">{format(r.value)}</span>
      {/each}
    </div>
  {/if}
</div>
