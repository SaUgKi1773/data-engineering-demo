...
select tournament from (
  ...
  from ligamx.mart_prediction_facts
...
select team_name from (
  ...
  from ligamx.mart_prediction_facts
  where match_id is not null
    and tournament = '${inputs.season.value}'
...
-- ignores the team filter, like the official table would. Only the 17 regular
-- rounds count — the liguilla awards no points and is not projected here.
--
-- Points already won come from the standings mart, not from the prediction
-- fact. The model started predicting Liga MX partway into the tournament, so
-- the earliest rounds have results but no prediction row; reading actuals off
-- the prediction fact would silently drop those points from every total.
with actual as (
    select
        team_short_name,
...
select
    (select max(r) from (
        select max(round) as r from ligamx.mart_standings
        where tournament = '${inputs.season.value}' and result in ('Win', 'Draw', 'Loss')
        union all
        select max(round_order) from ligamx.mart_prediction_facts
        where match_id is not null and tournament = '${inputs.season.value}'
          and round_order <= 17
    ))                                                                        as as_of_round,
...
```sql race
-- Solid line: actual points only, accumulated over played regular rounds and
-- anchored at (0, 0) so every team starts the race at the origin.
--
-- Read from the standings mart rather than the prediction fact. The model only
-- began predicting Liga MX partway into the tournament, so the opening rounds
...
with team_list as (
    ...
    -- ball is kicked the mart is empty for the tournament, so anchoring on it
    ...
    from (
        select team_name from ligamx.mart_standings
        where tournament = '${inputs.season.value}'
        union all
        select team_name from ligamx.mart_prediction_facts
        where match_id is not null
          and tournament = '${inputs.season.value}'
    )
...
from (
    select
        ...
        sum(points_earned) over (partition by team_name order by round)::int as cumulative_points,
        sum(goals_scored - goals_conceded)
            over (partition by team_name order by round)::int                as cumulative_gd
    from ligamx.mart_standings
    where tournament = '${inputs.season.value}'
      and result in ('Win', 'Draw', 'Loss')
...
actual as (
    select
        ...
        coalesce(max(round), 0)                         as last_played_round
    from ligamx.mart_standings
    where tournament = '${inputs.season.value}'
      and result in ('Win', 'Draw', 'Loss')
      and ('All Teams' in ${inputs.team.value} or team_name in ${inputs.team.value})
...
where match_id is not null
  ...
  and ('All Teams' in ${inputs.team.value}
       or team_name in ${inputs.team.value}
       or opponent_team_name in ${inputs.team.value})
group by round_order, outcome
order by round_order, outcome
```
...
<p style="font-size:0.8125rem;color:#6b7280;margin:0 0 1.5rem 0;">Before every fixture, the match model publishes win, draw and loss probabilities. Predictions are locked two days before the match and never revised; once a match is completed, the prediction is evaluated against the actual result. This page shows the current projection, upcoming predictions, and the model's accuracy to date. Only the 17 regular rounds are projected — the liguilla awards no points and is decided over two legs.</p>
...
<LineChart
    ...
    echartsOptions={{tooltip: {formatter: (function() { const gdOf = {}; for (const r of race) gdOf[r.team_name + '|' + r.round] = Number(r.cumulative_gd); for (const r of race_forecast) { const k = r.team_name + '|' + r.round; if (!(k in gdOf)) gdOf[k] = Number(r.cumulative_gd); } return function(params) { const seen = new Set(); const uniq = params.filter(p => { const nm = p.seriesName.replace(' (forecast)', ''); if (p.value == null || p.value[1] == null || seen.has(nm)) return false; seen.add(nm); return true; }); if (uniq.length === 0) return ''; const round = uniq[0].value[0]; const gd = (p) => gdOf[p.seriesName.replace(' (forecast)', '') + '|' + p.value[0]] ?? 0; const sorted = [...uniq].sort((a, b) => (b.value[1] - a.value[1]) || (gd(b) - gd(a))); let out = '<span style="font-weight:600;">Round ' + round + '</span>'; for (const p of sorted) { const isFc = p.seriesName.includes(' (forecast)'); out += '<br><span style="font-size:11px;">' + p.marker + ' ' + p.seriesName.replace(' (forecast)', '') + '</span><span style="float:right;margin-left:10px;font-size:12px;' + (isFc ? 'font-style:italic;color:#9ca3af;' : '') + '">' + (isFc ? '~' : '') + p.value[1] + '</span>'; } return out; }; })()}, series: (function() { const solidTeams = [...new Set(race.map(r => r.team_name))]; const fcTeams = [...new Set(race_forecast.map(r => r.team_name))]; const allTeams = [...solidTeams]; for (const t of fcTeams) if (!allTeams.includes(t)) allTeams.push(t); const colorOf = {}; allTeams.forEach((t, i) => { colorOf[t] = teamPalette[i % teamPalette.length]; }); const byTeam = {}; for (const r of race_forecast) { (byTeam[r.team_name] = byTeam[r.team_name] || []).push([Number(r.round), Number(r.cumulative_points)]); } const base = solidTeams.map(() => ({lineStyle: {width: 2}})); const fc = fcTeams.map(t => ({name: t + ' (forecast)', type: 'line', showSymbol: false, color: colorOf[t], lineStyle: {type: 'dashed', width: 2}, data: byTeam[t]})); return base.concat(fc); })()}}
    legend=false
    chartAreaHeight=300
/>
