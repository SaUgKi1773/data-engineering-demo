...
select distinct team_name from (
    select home_team as team_name from ligamx.mart_upcoming where home_team is not null
    union
    select away_team as team_name from ligamx.mart_upcoming where away_team is not null
) order by team_name asc
```

```sql rounds
select distinct cast(round_order as int) as round_order, round_label
from ligamx.mart_upcoming
...
<p style="font-size:0.75rem;color:#6b7280;margin:0 0 1rem 0;font-style:italic;">Filter by team or round to narrow the fixtures. Click any match to open its preview — head-to-head history and recent form for both sides. Kick-off times are local Mexico City time (CST).</p>

<Dropdown data={teams} name=team value=team_name label=team_name order="team_name asc" multiple=true selectAllByDefault=true />
{#key rounds[0]?.round_order}
<Dropdown data={rounds} name=round value=round_order label=round_label title="Round" order="round_order asc" multiple=true defaultValue={rounds[0]?.round_order} />
{/key}
...
select
    ...
    p.draw_pct,
    p.away_pct
from ligamx.mart_upcoming u
left join ligamx.mart_upcoming_predictions p on p.match_id = u.match_id
where u.home_team is not null
  and u.round_order in ${inputs.round.value}
...
```sql round_title
select string_agg(round_display, ', ' order by round_order) as label
from (
    select distinct cast(round_order as int) as round_order, round_display
    from ligamx.mart_upcoming
    where home_team is not null
...
## Upcoming Fixtures — {round_title[0]?.label}
