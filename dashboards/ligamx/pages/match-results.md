...
select *,
    ...
    '<a href="/match-analysis?match=' || cast(match_id as varchar) || '&tournament=' || tournament || '&round=' || cast(cast(round_order as integer) as varchar) || '" style="color:#2563eb;font-weight:600;text-decoration:none;">' || match_name       || '</a>' as match_link,
...
<p style="font-size:0.75rem;color:#6b7280;margin:0 0 1rem 0;font-style:italic;">Standout clubs from this round — one team recognised per category based on the highest single-match figure. Liga MX publishes no player-level data, so the award is made at club level.</p>
