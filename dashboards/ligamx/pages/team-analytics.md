...
```sql venues
select distinct team_side from ligamx.mart_team_match order by team_side
```
...
<p style="font-size:0.75rem;color:#6b7280;margin:0 0 1rem 0;font-style:italic;">Pick a season and team to explore their profile. Everything responds to the additional filters — the header, KPIs, goals timeline, opponents, home/away, scheduling and match log — benchmarked against the previous tournament sliced the same way. Two references stay fixed: Last 5 results and Ranking, neither of which can be derived from a filtered set of matches. Ranking is the regular-season table position, which is what seeds the liguilla.</p>
...
prev_season as (
    select max(tournament) as tournament from ligamx.mart_team_match
    where tournament < '${inputs.season.value}'
      and team_name = '${inputs.team.value}'
),
prev as (
...
rank_cur as (
    ...
    from ligamx.mart_team_season
    where tournament = '${inputs.season.value}' and team_name = '${inputs.team.value}'
),
rank_prev as (
    select season_rank as prev_rank
    from ligamx.mart_team_season
    where tournament = (select tournament from prev_season) and team_name = '${inputs.team.value}'
)
select
    cur.goals_per_match,
...
from ligamx.mart_team_event_timing
where tournament = '${inputs.season.value}'
  and team_name = '${inputs.team.value}'
  and ('All Opponents' in ${inputs.opponent.value} OR opponent_team_name in ${inputs.opponent.value})
  and result in ${inputs.result.value}
...
from ligamx.mart_team_game_state
where tournament = '${inputs.season.value}'
  and team_name = '${inputs.team.value}'
  and ('All Opponents' in ${inputs.opponent.value} OR opponent_team_name in ${inputs.opponent.value})
  and result in ${inputs.result.value}
