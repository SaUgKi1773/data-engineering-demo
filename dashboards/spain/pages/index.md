...
sidebar: never
hide_toc: true
title: La Liga Analytics — Spanish football intelligence
hide_title: true
description: Free, open analytics for La Liga, the Spanish top flight — standings, match results, league and team intelligence, refreshed every night. A Krogvad Analytics Hub platform.
---
...
```sql summary
select * from spain.mart_home_summary
```
...
<div class="relative rounded-3xl overflow-hidden shadow-lg mb-8" style="background: linear-gradient(135deg, #40100a 0%, #AA151B 45%, #A8611A 100%); font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  <!-- pitch lines overlay -->
  ...
  <div class="relative px-6 py-7 xl:px-12 xl:py-6 text-center xl:text-left flex flex-col xl:flex-row items-center xl:justify-between gap-4 xl:gap-6">
    ...
    <div class="flex flex-col xl:flex-row items-center xl:items-center gap-3 xl:gap-5">
      <img src="{league[0].league_logo}" alt="La Liga" class="h-12 xl:h-14 w-auto flex-shrink-0" onerror="this.style.display='none'" />
      <div>
        <div class="flex items-center justify-center xl:justify-start gap-2 mb-1">
          <img src="{league[0].league_country_flag}" alt="Spain" class="h-3.5 rounded opacity-90" onerror="this.style.display='none'" />
    ...
    <div class="flex flex-col items-center xl:items-end gap-2">
      ...
      <!-- One table decides the title, so the club top of it is the leader
           while the season runs and the champion once it has ended. -->
