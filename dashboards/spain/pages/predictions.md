...
select season from (
  ...
  from spain.mart_prediction_facts
...
-- fact. The model started predicting La Liga partway into the season, so
...
select
    (select max(r) from (
        select max(round) as r from spain.mart_standings
        where season = '${inputs.season.value}' and result in ('Win', 'Draw', 'Loss')
        union all
        select max(round_order) from spain.mart_prediction_facts
...
-- began predicting La Liga partway into the season, so the opening rounds
