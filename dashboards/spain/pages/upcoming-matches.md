...
select distinct team_name from (
    select home_team as team_name from spain.mart_upcoming where home_team is not null
    union
    select away_team as team_name from spain.mart_upcoming where away_team is not null
) order by team_name asc
```

```sql rounds
select distinct cast(round_order as int) as round_order, round_label
from spain.mart_upcoming
...
<p style="font-size:0.75rem;color:#6b7280;margin:0 0 1rem 0;font-style:italic;">Filter by team or round to narrow the fixtures. Click any match to open its preview — head-to-head history and recent form for both sides. Kick-off times are local Spanish time (CET/CEST).</p>
...
select
    ...
    p.draw_pct,
    p.away_pct
from spain.mart_upcoming u
left join spain.mart_upcoming_predictions p on p.match_id = u.match_id
