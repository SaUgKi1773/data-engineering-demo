...
<details class="mb-6 rounded-xl border border-blue-100 bg-blue-50">
  <summary class="cursor-pointer px-4 py-3 text-sm font-semibold text-blue-700 flex items-center gap-2">
    ℹ️ How does the La Liga season work?
  </summary>
  <div class="px-4 pb-4 pt-2 text-sm text-gray-700 space-y-3">
    ...
    <p><strong>The league is the same size every season.</strong> Twenty clubs, 38 rounds, 380 matches — so season totals compare directly from one year to the next.</p>
    <p><strong>Level on points is settled between the clubs, not by goal difference.</strong> Where two or more clubs finish on the same points, La Liga ranks them on their record against each other — a mini-table of just those clubs, points first and then goal difference within it — and only falls back to overall goal difference if that is level too.</p>
    <p><strong>At stake:</strong> the top of the table takes Spain's Champions League places, entering at the league phase rather than through qualifying, the places below go to the Europa and Conference Leagues, and the bottom three are relegated to the Segunda División.</p>
  </div>
</details>
...
-- La Liga separates clubs level on points by the mini-table between
