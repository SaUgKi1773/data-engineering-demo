...
select round, round_label from (
  ...
  from spain.mart_team_match
  where season = '${inputs.season.value}'
) order by ord
```

```sql venues
select distinct team_side from spain.mart_team_match order by team_side
```
...
prev_season as (
    select max(season) as season from spain.mart_team_match
    where season < '${inputs.season.value}'
      and team_name = '${inputs.team.value}'
),
prev as (
...
from spain.mart_team_event_timing
where season = '${inputs.season.value}'
  and team_name = '${inputs.team.value}'
  and ('All Opponents' in ${inputs.opponent.value} OR opponent_team_name in ${inputs.opponent.value})
  and result in ${inputs.result.value}
...
from spain.mart_team_game_state
where season = '${inputs.season.value}'
  and team_name = '${inputs.team.value}'
  and ('All Opponents' in ${inputs.opponent.value} OR opponent_team_name in ${inputs.opponent.value})
  and result in ${inputs.result.value}
