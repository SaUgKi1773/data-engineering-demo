...
-- this one reads the team fact directly - the Highlightly feed has no
-- player-level stat lines at all, but its team-level feed is deeper than
-- Denmark's. Two consequences:
...
--
-- NULL is never coerced to 0. Goals, shots, possession, passing, corners,
-- cards and fouls run back to 2020/21, but the deep stats (xG, big chances,
-- key passes, crosses, tackles, interceptions, clearances) effectively begin
-- in 2025/26 - 2024/25 carries a token few per cent of them and everything
-- earlier none at all. Before that they are genuinely unrecorded, and a 0
-- would claim a team made no tackles. The pages render those as an em dash.
WITH base AS (
    SELECT
        m.match_id,
...
SELECT
    ...
    max(is_current_season)                                                                     AS is_current_season,
