...
-- one row per (season, round, category).
--
-- This is the Highlightly answer to the Danish Players of the Week. The feed
-- carries no player-level data at all - no appearances, no ratings, no minutes
-- - so there is nobody to award. Its team-level feed is the deeper of the two,
-- so the award moves up a level: same six-card shelf, clubs instead of players.
--
...
-- effectively start in 2025/26, and a round with no cards has no card winner;
-- those cards render as an em dash rather than disappearing.
WITH base AS (
    SELECT
...
ranked AS (
    ...
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY season, round_order
...
LEFT JOIN picks p ON p.season  = r.season
                 AND p.round_order = r.round_order
                 AND p.sort_order  = c.sort_order
ORDER BY r.season, r.round_order, c.sort_order
