...
-- of League Intelligence. The page filters this on season, team, round,
...
-- this one reads the team fact directly: the Highlightly feed publishes no
-- player-level data at all. Two consequences run through the whole page:
--
...
--     passes) effectively begin in **2025/26**: 2024/25 carries a token few
--     per cent of them and the four seasons before it none at all. They stay
--     NULL, never 0 — a 0 would claim a team won no aerial duels. The page
--     renders them as an em dash.
--
-- Everything from possession, shots, passes, fouls, saves, corners, offsides
-- and cards is complete from 2020/21.
WITH pen AS (
    ...
    -- appear from 2024/25, so `penalty_missed` is NULL before that
    -- rather than 0 — otherwise every early season would report a
    ...
    WHERE e.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 119924)
...
SELECT
    ...
    CASE WHEN d.season_spain >= '2024/25'
         THEN COALESCE(pen.penalty_missed, 0) END   AS penalty_missed,
    -- deep-stat era only, effectively 2025/26 onwards
    f.expected_goals,
    f.expected_assists,
    ...
    f.attacks,
    -- La Liga never splits into championship and relegation groups the
    -- way the Danish season does, so this is a constant. Kept so the page's
    -- shared shape survives; the group legend and column simply never appear.
    'Regular Season'                                AS standings_type,
    -- Cumulative totals run in round order across the whole season.
    SUM(f.points_earned) OVER (
        PARTITION BY f.team_sk, d.season_spain
...
WHERE r.match_result IN ('Win', 'Draw', 'Loss')
  AND f.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 119924)  -- La Liga only
