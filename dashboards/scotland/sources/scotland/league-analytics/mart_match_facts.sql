...
SELECT
    ...
    d.is_current_season_scotland AS is_current_season,
    d.month_name,
    d.day_name,
    d.is_weekend,
    m.match_round_name,
    ...
    CASE
        WHEN MAX(CASE WHEN m.match_round_type = 'Championship Round' THEN 1 ELSE 0 END) OVER (PARTITION BY f.team_sk, d.season_scotland) = 1
            THEN 'Championship Group'
        WHEN MAX(CASE WHEN m.match_round_type = 'Relegation Round'   THEN 1 ELSE 0 END) OVER (PARTITION BY f.team_sk, d.season_scotland) = 1
