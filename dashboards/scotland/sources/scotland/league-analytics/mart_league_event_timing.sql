...
WHERE d.season_scotland >= '2020/21'
  AND e.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 501)  -- Premiership only
  AND r.match_result IN ('Win', 'Draw', 'Loss')
  AND mm.match_minute_sk > 0
  AND mm.minute_bucket != 'Extra Time'
GROUP BY ALL
ORDER BY d.season_scotland DESC, t.team_name, m.match_round_number, mm.minute_bucket_sort
