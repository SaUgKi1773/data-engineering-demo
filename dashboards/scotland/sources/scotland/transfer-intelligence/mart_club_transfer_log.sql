...
WITH team_match_years AS (
    ...
    WHERE m.team_sk > 0
      -- fct_team_transfers has no league_sk; scope to clubs via their Premiership matches
      AND m.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 501)
)
SELECT
    f.transfer_id,
