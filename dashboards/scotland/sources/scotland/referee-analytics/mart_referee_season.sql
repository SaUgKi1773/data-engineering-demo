...
var_agg AS (
    ...
    WHERE d.season_scotland >= '2020/21'
      AND ve.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 501)  -- Premiership only
      AND ve.referee_sk > 0
      AND et.event_group = 'VAR'
    GROUP BY d.season_scotland, ref.referee_common_name
)
SELECT
    d.season_scotland AS season,
    ref.referee_common_name                                                                AS referee_name,
...
LEFT JOIN var_agg                         v   ON v.season          = d.season_scotland
...
ORDER BY d.season_scotland DESC, matches_managed DESC
