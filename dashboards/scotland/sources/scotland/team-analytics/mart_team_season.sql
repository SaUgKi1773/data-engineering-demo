...
per_match AS (
    SELECT
        ...
        t.team_name,
        t.team_logo,
        MAX(dc.coach_name) OVER (PARTITION BY f.team_sk, d.season_scotland)  AS coach_name,
        m.match_id,
        ...
        CASE
            WHEN MAX(CASE WHEN m.match_round_type = 'Championship Round' THEN 1 ELSE 0 END)
                 OVER (PARTITION BY f.team_sk, d.season_scotland) = 1 THEN 'Championship Group'
            WHEN MAX(CASE WHEN m.match_round_type = 'Relegation Round'   THEN 1 ELSE 0 END)
                 OVER (PARTITION BY f.team_sk, d.season_scotland) = 1 THEN 'Relegation Group'
            ELSE 'Regular Season'
        END                                     AS standings_type,
        SUM(f.points_earned) OVER (
            PARTITION BY f.team_sk, d.season_scotland
...
avg_age_calc AS (
    SELECT
        d.season_scotland AS season,
        f.team_sk,
        ROUND(AVG(CAST(LEFT(d.season_scotland, 4) AS INTEGER) - YEAR(p.player_birth_date)), 1) AS avg_age
    FROM (
        SELECT DISTINCT
    ...
    GROUP BY d.season_scotland, f.team_sk
)
SELECT
    r.season,
    r.team_name,
