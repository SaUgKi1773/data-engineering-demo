...
from scotland.mart_team_season
...
```sql venues
select distinct team_side from scotland.mart_team_match order by team_side
```
...
select
    ...
    (select coach_name from scotland.mart_team_season s
...
prev_season as (
    select max(season) as season from scotland.mart_team_match
    where season < '${inputs.season.value}'
      and team_name = '${inputs.team.value}'
),
prev as (
...
```sql avg_age
with prev_season as (
    select max(season) as season from scotland.mart_team_season
    where season < '${inputs.season.value}'
)
select
    cur.avg_age,
    prev.avg_age as prev_avg_age
from scotland.mart_team_season cur
left join scotland.mart_team_season prev
