...
```sql team_lookup
select distinct club, club_code from scotland.mart_club_transfer_log
```
...
from scotland.mart_club_transfer_log
...
select club from (
  ...
  union all
  select distinct club, 1 as ord from scotland.mart_club_transfer_log
) order by ord, club
```

```sql directions
select distinct direction from scotland.mart_club_transfer_log order by direction
```

```sql types
select distinct transfer_type from scotland.mart_club_transfer_log order by transfer_type
```

```sql statuses
select distinct transfer_status from scotland.mart_club_transfer_log order by transfer_status
```
...
```sql by_club
with f as (
  select * from scotland.mart_club_transfer_log
