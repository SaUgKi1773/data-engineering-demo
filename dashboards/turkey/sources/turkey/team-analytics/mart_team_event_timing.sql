-- Event timing at team-match x 15-minute-bucket grain, carrying the page's
-- slice dimensions. Every match contributes one row per side and bucket:
-- goals_for / cards / substitutions count the row's team, goals_against counts
-- the opponent. The page aggregates to buckets at query time.
--
-- Extra Time is excluded; see the note in mart_league_event_timing.
SELECT
...
GROUP BY ALL
ORDER BY d.season_turkey DESC, t.team_name, round_order, mm.minute_bucket_sort
