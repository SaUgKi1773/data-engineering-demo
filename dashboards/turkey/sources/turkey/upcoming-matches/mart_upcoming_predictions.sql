-- Locked pre-match probabilities for pending fixtures, one row per match,
-- joined onto the fixture list by match_id. Home perspective only: the home
-- row carries the whole match-level prediction.
WITH base AS (
    SELECT
        m.match_id,
    ...
    WHERE p.team_side_sk = 1  -- home perspective carries the full match-level prediction
      ...
      AND p.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 173537)  -- Süper Lig only
)
SELECT * FROM base
UNION ALL
