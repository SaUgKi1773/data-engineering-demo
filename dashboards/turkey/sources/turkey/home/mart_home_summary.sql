-- Headline numbers for the latest Süper Lig season, on the same column
-- contract as the Danish and Scottish mart_home_summary so the home page
-- stays a twin of theirs.
--
-- The Süper Lig is a plain double round-robin: one table, no split, no
...
WITH matches AS (
    SELECT
        ...
        d.is_current_season_turkey AS season_is_live,
        d.date,
        m.match_id,
        t.team_name,
        t.team_short_name,
    ...
    WHERE r.match_result IN ('Win', 'Draw', 'Loss')
      AND f.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 173537)
