...
-- at query time.
--
-- Extra Time is excluded: every Süper Lig match is a league match played to 90
...
WHERE e.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 173537)  -- Süper Lig only
