...
WITH pen AS (
    ...
    WHERE e.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 173537)
...
SELECT
    ...
    CASE WHEN d.season_turkey >= '2024/25'
    ...
    f.attacks,
    -- The Süper Lig never splits into championship and relegation groups the
    ...
    SUM(f.points_earned) OVER (
        PARTITION BY f.team_sk, d.season_turkey
