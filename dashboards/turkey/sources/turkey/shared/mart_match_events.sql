...
-- timeline: goals, cards, substitutions, VAR reviews and missed penalties,
-- each with the minute, the acting side and player, and the running score
-- after the event. Ordered chronologically (minute, then stoppage offset,
-- then within-group sequence).
--
-- Missed penalties are carried here but not on the Danish site: the Highlightly
-- feed publishes them (from 2024/25) and the Danish one does not.
SELECT
    m.match_id,
    mm.minute_of_match,
    mm.minute_label,
...
WHERE f.league_sk = (SELECT league_sk FROM superligaen.gold.dim_league WHERE league_id = 173537)  -- Süper Lig only
  AND d.season_turkey IS NOT NULL
  AND et.event_group IN ('Goal', 'Card', 'Substitution', 'VAR', 'Missed Penalty')
