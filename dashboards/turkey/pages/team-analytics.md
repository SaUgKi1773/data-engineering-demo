...
select round, round_label from (
  ...
  from turkey.mart_team_match
  where season = '${inputs.season.value}'
) order by ord
```

```sql venues
select distinct team_side from turkey.mart_team_match order by team_side
```
...
<p style="font-size:0.75rem;color:#6b7280;margin:0 0 1rem 0;font-style:italic;">Pick a season and team to explore their profile. Everything responds to the additional filters — the header, KPIs, goals timeline, opponents, home/away, scheduling and match log — benchmarked against the previous season sliced the same way. Two references stay fixed: Last 5 results and Ranking, neither of which can be derived from a filtered set of matches. Ranking is the club's position in the league table.</p>
...
prev_season as (
    select max(season) as season from turkey.mart_team_match
    where season < '${inputs.season.value}'
      and team_name = '${inputs.team.value}'
),
prev as (
...
```sql goals_per_match
-- One point per match played, in date order.
with played as (
    select
        match_date,
        round_display,
        ...
        result,
        'R' || round_order::int::varchar as round_code
    from turkey.mart_team_match
    where season = '${inputs.season.value}'
      and team_name = '${inputs.team.value}'
      and result in ${inputs.result.value}
...
<p style="font-size:0.75rem;color:#6b7280;margin:0 0 1rem 0;font-style:italic;">Goals scored and conceded in every match of the season, in the order they were played. Hover a match to see the opponent.</p>
...
from turkey.mart_team_event_timing
where season = '${inputs.season.value}'
  and team_name = '${inputs.team.value}'
  and ('All Opponents' in ${inputs.opponent.value} OR opponent_team_name in ${inputs.opponent.value})
  and result in ${inputs.result.value}
...
from turkey.mart_team_game_state
where season = '${inputs.season.value}'
  and team_name = '${inputs.team.value}'
  and ('All Opponents' in ${inputs.opponent.value} OR opponent_team_name in ${inputs.opponent.value})
  and result in ${inputs.result.value}
