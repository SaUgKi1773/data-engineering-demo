...
select season from (
  ...
  from turkey.mart_prediction_facts
...
-- fact. The model started predicting the Süper Lig partway into the season, so
...
select
    (select max(r) from (
        select max(round) as r from turkey.mart_standings
        where season = '${inputs.season.value}' and result in ('Win', 'Draw', 'Loss')
        union all
        select max(round_order) from turkey.mart_prediction_facts
...
-- began predicting the Süper Lig partway into the season, so the opening rounds
...
with team_list as (
    ...
    from (
        select team_name from turkey.mart_standings
        where season = '${inputs.season.value}'
        union all
        select team_name from turkey.mart_prediction_facts
        where match_id is not null
          and season = '${inputs.season.value}'
    )
