...
identically. The tables already exist in prod from the first Liga MX load;
this schema is byte-compatible with them.

Three tables:
  highlightly__matches        one row per match, from the season list endpoint.
                              Cheap and always complete — the schedule/results
                              backbone, refreshed per (league, season).
  ...
  highlightly__standings      one row per standings group, per (league, season).
                              (Rows from the first, removed implementation hold
                              the whole {groups, league} envelope instead; any
                              refresh of that (league, season) replaces them.)
...
def pending_detail_matches(conn: duckdb.DuckDBPyConnection, league_id: int,
    ...
    from_date/to_date narrow the selection to a window — manual backfills seed
    history a window at a time, and the nightly run passes a rolling
    last-N-days window the same way the Sportmonks ingest does.
    """
    if not seasons:
        return []
