...
-----
  ...
  python run.py --leagues 119924                              # one league only
...
match details (one call per finished match — the expensive part).
  ...
  --leagues               league ids to run; default is every league in
                          config.LEAGUES.
...
The client reads the remaining quota from every response and stops cleanly
when it runs out, so a large window is safe to ask for: it fetches what it
can — leagues interleaved fairly — and the next run resumes. Nothing stores a
cursor: the outstanding work is derived from the data (finished matches with
no detail row). A paid key reports its larger quota through the same header,
so a one-shot historical backfill needs no code change.
"""
...
def main() -> None:
    ...
    parser.add_argument("--leagues", default=None, metavar="ID1,ID2",
                        help="Comma-separated league ids ("
                             + ", ".join(f"{i} = {v['name']}" for i, v in LEAGUES.items())
    ...
    if args.leagues:
        ...
        if unknown:
            log.error("Unknown league ids %s — configured: %s", unknown,
                      {i: v["name"] for i, v in LEAGUES.items()})
            sys.exit(2)
