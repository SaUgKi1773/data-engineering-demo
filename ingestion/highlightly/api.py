...
def get_match_list(league_id: int, season: int) -> list:
    """
    Every match in a season, paginated. ~340-380 matches -> 4 calls.

    BudgetExhausted mid-pagination propagates to the caller — the partial list
    is discarded and the season's existing bronze rows stay untouched, so an
    underfunded run never leaves a season half-listed.
    """
    out, offset = [], 0
    while True:
        payload = get("/matches", {
        ...
        if not batch or (total is not None and offset >= total) or len(batch) < PAGE_LIMIT:
            break
    log.info("league %s season %s: %d matches listed", league_id, season, len(out))
    return out
...
def get_standings(league_id: int, season: int) -> list:
    """
    Standings groups for a season (Liga MX answers two: Apertura and Clausura).
    The response is a {groups, league} envelope, NOT the usual {data, ...} one;
    unwrap it so bronze gets one row per group and the group name (e.g.
    "Liga MX: Apertura") survives at the top level of raw_json.
    A season with no table yet (fixtures published, nothing played) may answer
    4xx — that is absence of data, not an error.
    """
    try:
        payload = get("/standings", {"leagueId": league_id, "season": season})
        if isinstance(payload, dict) and "groups" in payload:
            return payload["groups"] or []
        return _rows(payload)
    except requests.HTTPError as exc:
        if exc.response is not None and 400 <= exc.response.status_code < 500:
            log.info("league %s season %s: no standings available (%d)",
                     league_id, season, exc.response.status_code)
            return []
        raise
