...
CREATE TABLE IF NOT EXISTS {db}.silver.fixture_statistics AS
SELECT * FROM (
    SELECT
        s.fixture_id,
        ...
        s.ingested_at
    FROM {db}.bronze.api_football__fixture_statistics s
...
DELETE FROM {db}.silver.fixture_statistics WHERE {delete_filter};

INSERT INTO {db}.silver.fixture_statistics
SELECT * FROM (
    SELECT
        s.fixture_id,
        (f.raw_json->>'$.fixture.date')::TIMESTAMPTZ  AS kick_off,
