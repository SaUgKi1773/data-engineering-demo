{{
    config(
        ...
        post_hook=[
            ...
            "INSERT INTO {{ this }} SELECT * FROM (VALUES (-1, NULL::INTEGER, 'Unknown League', 'Unknown League Type', NULL::VARCHAR, 'Unknown League Country', 'UNK', NULL::VARCHAR, 'UNK', 'Unknown League Sub Type', NULL::BOOLEAN, NULL::VARCHAR), (-2, NULL::INTEGER, 'Not Applicable League', 'Not Applicable League Type', NULL::VARCHAR, 'Not Applicable League Country', 'N/A', NULL::VARCHAR, 'N/A', 'Not Applicable League Sub Type', NULL::BOOLEAN, NULL::VARCHAR)) t(league_sk, league_id, league_name, league_type, league_logo, league_country, league_country_code, league_country_flag, league_short_code, league_sub_type, league_is_active, _source)"
        ]
    )
}}

-- src exists so _source is a real column the surrogate-key ordering can use:
-- a window ORDER BY cannot reference a SELECT alias, and ordering by a bare
-- literal would silently stop being source-aware the moment a second feed
-- lands here.
WITH src AS (
    SELECT
        l.id              AS league_id,
...
SELECT
    ...
    (SELECT COALESCE(MAX(league_sk), 0) FROM {{ this }} WHERE league_sk > 0)
        + ROW_NUMBER() OVER (ORDER BY league_id, _source) AS league_sk,
    {% else %}
    ROW_NUMBER() OVER (ORDER BY league_id, _source) AS league_sk,
    {% endif %}
    league_id,
