...
src AS (
    SELECT
        ...
        CASE mt.location WHEN 'home' THEN s.home_goals_exp ELSE s.away_goals_exp END AS predicted_goals_scored,
        CASE mt.location WHEN 'home' THEN s.away_goals_exp ELSE s.home_goals_exp END AS predicted_goals_conceded,
        s.model_version,
        s.predicted_at
...
SELECT
    ...
    src.predicted_goals_scored,
    src.predicted_goals_conceded,
    ROUND(3 * src.win_probability + src.draw_probability, 4) AS predicted_points,
    src.model_version,
    src.predicted_at
FROM src
