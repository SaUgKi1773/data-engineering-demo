...
-- each within [0, 1], the three summing to 1, and expected_points consistent
-- with them (3·win + 1·draw). Violations mean producer garbage reached gold
-- and must block the publish.
SELECT match_sk, team_side_sk, win_probability, draw_probability, loss_probability, expected_points
...
WHERE win_probability  NOT BETWEEN 0 AND 1
   ...
   OR ABS(expected_points - (3 * win_probability + draw_probability)) > 0.001
